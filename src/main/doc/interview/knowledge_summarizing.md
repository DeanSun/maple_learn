...
6. 微服务
    ...
    + 微服务使用模式
        ...
        + 命令与查询职责隔离（CQRS）：
            + 使用CQRS模式，主要的好处是可以有更清晰的领域模型，根据操作的不同方式，使用不同的领域模型。还可以进行读写优化，从而实现更好的性能。
        + 断路器：
            + 使用断路器对故障服务进行隔离。断路器有三种状态：关闭、打开、半开。当服务出现故障的时候，通过断路器阻断对故障服务的调用。避免它的故障扩散开来。在Spring Cloud中可以使用Hystrix实现断路器。
        + 超时：
            + 微服务调用的时候超时如何设置：
                + 通常情况下设置超时时间，上游调用者的超时时间是下游调用者超时时间的总和，如果是同一个时间就是毫无意义。
    + 使用微服务最重要的就是要做好业务的模块化设计，模块之间要低耦合，高聚合，模块之间的依赖关系要清晰简单。只有这样的模块化设计，才能够构建出良好的微服务架构。

7. 高性能系统架构
    + 性能测试：性能测试是性能优化的前提和基础，也是性能优化结果的检查和度量标准。
        + 经典名句：你不能优化一个你未经测试的系统，你也不能优化一个你不了解的系统。
        + 性能标准有主管和客观两个视角。
            + 主观标准是说使用者在使用上的快慢，用户在使用系统的时候，他主观上感觉快还是慢，那么就会得到一个性能是好还是差的主观标准。
            + 客观标准，也就是客观上性能指标到底是好还是坏。
        + 客观性能指标：主要有响应时间、并发数、吞吐量以及性能计数器。
            + 响应时间：所谓响应时间是指引用系统从发去请求开始到收到最后相应数据所需要的时间。响应时间是系统最重要的性能指标，最直接地反映了系统的快慢。
            + 并发数：并发数是指系统同时处理的请求书。
            + 吞吐量：吞吐量是指单位时间内系统处理请求的数量，体现的是系统的处理能力。我们一般使用的每秒的请求数、每秒的事务数这样的一些指标来衡量，也就是HPS、TPS这些，HPS是每秒的HTTP数目，TPS是每秒的事务数。还可以用QPS,即每秒的查询书来表示。总之就是单位时间内处理的请求数目。
            + 吞吐量、响应时间和并发数三者之间是有关联性的，响应时间足够快，那么单位时间的吞吐量也会相应的提高。
            + 性能计数器：服务器或者操作系统的一些指标数据，包括系统负载System Load、对象和线程数、内存使用、CPU使用、磁盘和I/O使用等指标。这些指标是系统监控的重要参数，反映系统负载和处理能力的一些关键指标，通常这些数据和指标是强相关的。
        + 性能测试、负载测试、压力测试和稳定性测试
        + 性能特性曲线：
            + 吞吐量特性曲线：在性能测试阶段，随着访问的逐渐增加，系统资源的消耗在不断的增加，TPS是在不断上升的;到了负载阶段，这种上升的斜率会变小，最后到达系统负载最高点，超过最高点系统便会崩溃；压力测试阶段，吞吐量会下降，是因为更多的客户进行请求，但是系统无法响应处理这些请求，所以TPS在下降中。
            + 响应时间特性曲线：和TPS相对应的响应时间则是相反的。随着并发用户数的增加，以及系统资源的不断消耗，系统的响应时间在不断增加。
        + 分层优化系统性能：
            + 机房与骨干网络的性能优化：主要手段就是采用异地多活的多机房假设，同时为了联通异地多活的多机房架构，会建设自己专用的骨干网络，并且自动进行CDN建设。
            + 对服务器内的硬件进行优化：系统伸缩有垂直伸缩和水平伸缩。互联网主要使用的是水平伸缩，这样成本比较低。垂直伸缩在某些时候也是不能忽略的，在预算可以的情况下可以适当考虑垂直伸缩带来的性能优化。例如机械硬盘和SSD硬盘。
            + 操作系统的性能优化：
            + 虚拟机的性能优化：
            + 基础组件的性能优化：
            + 软件架构的性能优化：
                + 缓存：通过缓存可以减少数据库的负载压力。减少了数据库的负载压力，另一方面可以使数据库提供更多的数据访问，支撑更大的系统访问压力，从而提升系统的整体性能。
                + 异步：通过分布式消息队列，使系统不同应用之间、不同服务之间异步调用，可以使调用者尽快的返回客户响应，使系统得到更好的响应特性。同时分布式消息队列的异步架构，还有削峰填谷的作用，在访问高峰期，通过异步将用户请求数据写入到消息队列中，在访问低谷的时候还在继续消费处理这些消息，避免对数据库等产生较大的负载访问压力，使系统能够维持在一个较好的响应曲线上。
                + 集群：通过负载均衡的手段，将多个应用服务器构建一个集群，共同提供服务，以提高系统整个的处理能力，提高系统的响应能力。
            + 性能优化的最底层才是软件代码的性能优化：
                + 遵循面向对象的设计原则和设计模式编程，避免烂代码
                + 并发编程，可以在程序并发运行的时候使用多线程。
                + 资源的复用：对于一些比较昂贵的资源。比如说线程或者数据库链接，我们要提供线程池或者数据库连接池来向外部提供服务。
                + 异步编程：在程序内部使用消费者、生产者，以队列数据结构的方式，实现程序内的异步架构，以此来提高系统性能。
                + 使用正确的数据结构进行编程：要熟悉数组、链表、栈、队列、Hash表、树等常用数据结构、熟悉他们的特性、优缺点以及使用场景，正确地使用它们来管理和访问程序数据。
    + 性能优化
        + 硬件优化
        + 中间件优化
        + 架构优化
...
8. 高可用系统架构
    + 互联网系统可用性度量：即如何用指标来衡量系统的可用性，以及进行可用性管理的一些手段
    + 高可用架构策略：主要包括负载均衡、备份与失效转移、消息队列隔离、限流与降级、异地多活这几种架构方法。
    + 高可用运维，如何在开发测试发布以及系统运行中，保障系统的高可用，包括自动化部署，自动化监控，自动化测试，预发布测试。
    + 互联网可用性的度量：
        + 互联网用常用多个个9来说明互联网应用的可用性。2个9表示系统基本可用，3个9是较高可用，4个9就是具有恢复能力的高可用，5个9指极高的高可用。
    + 故障分类：
        + 事故级故障：严重故障，网站整体不可用
        + A类故障：网站访问不顺畅或核心功能不可用
        + B类故障：非核心功能不可用，或核心功能少数用户不可用
        + C类故障：以上故障以外的其他故障
    + 故障处理流程和处理时间
    + 大型系统高可用的一般策略：
        + 负载均衡：应用服务器的负载均衡。负载均衡核心要解决的就是通过一个负载均衡服务器，将用户的请求分发给多个应用服务器，将多个应用服务器构建成一个集群，共同对外提供服务。这样的架构可以提高系统的处理能力，以解决高并发用于请求下的系统性能问题。
            + 负载均衡的实现手段：
                + HTTP重定向负载均衡
                + DNS负载均衡：有两方面的问题：一方面它依然是要将Web服务器的IP地址暴露给浏览器，产生安全问题；另一方面，很多时候，DNS域名服务器是互联网应用系统之外的一个服务器，它由域名解析服务商提供，不在我们的控制范围之内，所以当我们的服务器不可用的时候，DNS解析服务器并不知道，它依然会将用户请求分发过来。
                + 反向代理负载均衡：反向代理是工作在HTTP协议层上的一个服务器，所以代理的也是HTTP请求和响应。而HTTP协议相对来说，作为网络第七层的一个协议，它的协议比较重，效率比较低，所以反向代理负载均衡通常在小规模的互联网系统上，只有几台或者十几台服务器的规模。
                + IP层负载均衡（四层负载均衡）：IP层是网络通讯协议的第四层，所以有的时候叫做四层负载均衡。工作原理就是，当用户的请求到达负载均衡服务器以后，负载均衡服务器会拿到TCP/IP的数据包，对数据包的IP进行转换，修改IP地址，将其修改为WEB服务器的IP地址，然后将数据包重新发送出去。
                + 数据链路层负载均衡：负载均衡服务器并不修改数据包的IP地址，而是修改网卡的MAC地址。而应用服务器和负载均衡服务器使用的相同的IP地址，这样IP路由就不会受到影响，但是网卡会根据自己的MAC地址选择负载均衡发送到自己的网卡的数据包，交给对应的应用服务器去处理，处理结束以后，当它把响应的数据发回到网络的时候，因为IP地址没有修改过，所以这个响应就直接到达用户的浏览器，而不会再经过负载均衡服务器。
                    + 这种模式也被叫做三角模式。数据链路层的负载均衡是目前大互联网系统中使用的最多的负载均衡方案。
        + 数据库复制与失效转移：
        + 消息队列隔离
        + 限流和降级
        + 异地多活机房架构
...
9. 系统的安全架构设计：
    + Web攻击与防护：
        + XSS攻击：XSS跨站点脚本攻击。就是通过一个非法的浏览器脚本，让用户跨站点去执行，从而达到攻击的效果。
            + URL钓鱼
            + 直接攻击
        + XSS攻击防御：
            + 消毒：检查用户提交的请求中是否含有可执行的脚本，因为大部分的脚本都需要用尖括号等脚本语法，因此大部分的攻击都包含尖括号这样的脚本语法，所以通过HTML转义的方式，这样这种只能正常浏览但是不能被执行，所以就无法进行攻击。
            + HttpOnly：由于脚本攻击需要使用cookie，所以在cookie上设置HttpOnly的属性，使恶意脚本无法获取Cookie，避免被攻击脚本攻击。
        + SQL注入
        + SQL攻击防御：
            + 消毒：对请求数据进行正则表达式匹配,对非法参数进行相应的处理。
            + 参数绑定：目前很多Web数据库访问框架，比如MyBatis，Hibernate等ORM框架，都支持SQL预编译和参数绑定，能够避免SQL注入攻击。
        + CSRF攻击：跨站请求伪造攻击。比如用户先登录受信的服务器，然后访问了被攻击的服务器。被攻击了的服务器返回的响应里面包含着用户访问受信任的服务器请求，用户在不知情的情况下执行了请求，从而达到了攻击者的目的。
        + CSRF攻击防御：
            + 表单Token
            + 验证码
    + Web应用防火墙：大部分的攻击都可以在请求和响应阶段进行拦截处理，所以不用每个程序自己去独立处理。在应用网关或者在请求接入的时候，可以设置一个统一的Web应用服务器，对所有的请求响应进行拦截处理。
    + 信息加解密：
        + 单向散列加密: 进行单向加密，不能进行解密。强化班就是在加密过程中加盐。
        + 对称加密：
        + 非对称加密：
    + 信息过滤与反垃圾：
        + 贝叶斯分类算法
...
10. 分布式存储系统Doris
    + 进行海量数据存储；要求能够线性伸缩；系统要高可用，要能够自动的容错和故障转移；要高性能，要在高并发的情况下，依然保持较低的响应时间；具备灵活的扩展能力；有较低的运维成本。
    + 整体框架包括三部分：
        + 客户端：Doris提供给客户SDK包，客户可以使用Doris SDK进行分布式的数据读写操作，Doris支持存储Key/Value这样的KV数据结构，跟缓存一样。
        + Administration：可以通过控制台进行集群的故障管理和扩容管理。
        + Data Server数据存储：Data Server也是数据真正存储的地方，Doris对数据进行分片存储。
    + Doris数据分区架构和分区算法：
        + Doris高可用：主要解决两种情况，一种就是系统临时出现故障的时候，另一种就是系统永久失效。

11. 反应式编程框架Flower：
    + 有四个特性：
        + 即时响应：应用的调用者可以即时得到相应，无需等到整个应用程序执行完毕，也就是说应用调用是非阻塞的。
...
1. JMM:
    + JMM就是Java内存模型。主要是定义程序中的变量的访问规则，所有的共享变量都存储在主存中共享。每个线程有自己的工作内存，工作内存中保存的是主存中的变量的副本，线程对变量的读写等操作必须在自己的工作内存中进行，而不能直接读写内存中的变量。
    + 原子性：保证对除long和double外的基础数据类型的读写操作是原子性的。另外同步关键字synchronized也可以提供原子性保证。同步指令是通过Java的两个高级的字节码指令monitorenter和monitorexit来保证的。
    + 可见性：一个是通过synchronized，一个是通过volatile。volatile强制变量的赋值同步刷新到主存中，强制变量的读取会从主内存重新加载，保证不同的线程总是能够看到该变量的最新值。
    + 有序性：主要通过volatile和一些列happens-before原则。volatile的另一个作用就是阻止指令重排序，这样就可以保证变量读取的有序性。
        + happen-before：
            + 规则：
                + 程序顺序规则：一个线程中的每一个操作，happens-before于该线程栈中的任意后续的操作。
                ...
                + start()规则：线程A执行操作ThreadB.start(),那么A线程的ThreadB.start() happens-before于B线程的任意操作。
                + join()规则：线程A执行操作ThreadB.join()并成功返回后，线程B的任意操作都将对线程A可见。

2. 类加载机制：
    + 类的加载指将编译好的Class类文件中的字节码读入到内存中，将其放在方法区内并创建对应的Class对象。
    + 类的加载分为加载、链接、初始化，其中链接又包括验证、准备、解析。
        + 加载：文件到内存的过程。通过类的完全限定名查找此类的字节码文件，并利用字节码文件创建一个Class对象。
        + 验证：对类文件进行验证。目的在于保证Class文件符合当前虚拟机要求，不会危害当前虚拟机自身安全。主要包括四种：文件格式校验、元数据校验、字节码校验、符号引用校验。
        + 准备：进行内存分配。为类变量也就是类中由static修饰的变量分配内存，并且设置初始值。初始化的值是0或者null，不是代码里面设置的具体值，代码中设置的值是在初始化阶段完成的。这里不包含用final修饰的变量，因为final在编译的时候就已经进行了分配。
        + 解析：主要是解析字段、接口、方法。主要是将常量池中的引用替换为直接引用的过程。直接引用就是直接指向目标的指正或者相对偏移量等。
        + 初始化：主要完成静态块执行和静态变量的赋值。这是类加载的最后阶段，若被加载的父类没有被初始化就先初始化父类。
    + 类初始化触发的条件：
        + 创建类的实例
        + 访问类的静态方法或者静态变量
        + Class.forName()反射类时
...
1. JVM内存模型
    + 线程独占：栈(Stack)、本地方法栈(Native Method Stack)、程序技术栈(Program Counter Register)
        + 栈：也叫方法栈，是线程私有的，线程在执行每个方法的时候都会创建一个栈针，用来存储局部变量表、操作栈、动态链接、方法出口等信息。调用方法的时候执行入站，方法返回时执行出栈。
        + 本地方法栈：与栈类似，也是用来保存线程执行方法时的信息，不同的是，执行Java方法使用栈，而执行native方法使用本地方法栈。
        + 程序技术栈：程序计数器保存当前程序所执行的字节码位置，每个线程工作都有一个独立的计数器。程序计数器为执行Java方法服务，执行Native方法时，程序计数器为空
    + 线程共享：堆(Heap)、方法区(Method Area)
        + 堆：JVM管理的内存中最大的一块，堆被所有线程共享，目的是为了存放对象实例，几乎所有的对象都在这里进行分配。当堆空间不足的时候会抛出OOM的异常。根据对象存活的周期不同，JVM把堆内存进行分代管理，由垃圾回收器来进行对象的回收管理。
        + 方法区：各线程共享的内存区域，又称非堆区。用于存放虚拟机加载的类信息、常量、静态变量、即时编译器编译后的代码等数据，jdk1.7中的永久代和1.8中的metaspace都是方法区的一种实现。

2. GC算法
    + CMS算法：
        + 1.7之前最主流的GC垃圾回收算法。CMS使用的是标记清楚，优点是并发收集，停顿小。
        + ![avatar](../interview/img/CMS_process_flow.png)
    + G1算法：
        + 1.9以后成为JVM的默认垃圾回收算法，G1的特点就是保持高回收率的同时，减少停顿。
        + G1和CMS一样会遍历所有的对象，然后标记对象引用情况，在清除对象后会对区域进行复制移动整合碎片空间。
        + 年轻代回收：采用复制算法，并行进行收集，收集过程会STW.
        + 老年代回收：同时也会对年轻代进行回收。主要有四个过程：
            + 依然是初试标记阶段完成对跟对象的标记，这个过程是STW的。
            + 并发标记阶段，这个阶段和用户线程并行执行的。
            + 最终标记阶段，完成三色标记阶段
            + 复制/清除阶段，这个阶段会优先对可回收的Region进行回收，即Garbage First，这是G1的来由
        + 每次采用清理一部分的方法不是所有的Region区域，由此来保证每次GC停顿时间不会过长。
    + G1和CMS都是采用的三色标记法算法，ZGC和Shenandoah使用的是颜色指针。
    + ZGC算法：
        + jdk11中提供的高效的垃圾回收算法，ZGC针对大堆内存设计可以支持TB级别的堆，ZGC非常高效，能过做到10ms一下的停顿时间。
        + 特点：
            + 使用着色指针技术，64为平台上，一个指针可用位是64位，ZGC限制最大支持4TB的堆，这样寻址需要使用42位，剩下的22位就能够用来保存其他的信息，着色指针技术就是利用指针的额外信息位，在指针上对对象做着色。
            + 读屏障，ZGC使用读屏障来解决GC线程和应用线程并发修改对象状态的问题，而不是使用简单粗暴的通过STW来进行全局的锁定。使用读屏障只会在一定概率上在单对象上有减速。
            + 由于读屏障的作用，所以ZGC不需要使用STW，因此ZGC大部分时间都是在并发处理。
            + 基于Region，这与G1算法一样，不过虽然也分了Region，但是并没有进行分代。ZGC的Region不像G1那样是固定大小，而是动态决定Region的大小，Region可以动态创建和销毁。这样可以更好的对大对象进行管理。
            + 压缩整理。CMS算法清理对象时原地回收，会存在碎片问题。ZGC和G1一样，也会对回收后的Region进行移动合并，解决了碎片问题。
